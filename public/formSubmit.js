...
if (!window.formSubmitInitialized) {
  ...
  // ============================================================
  // 🆕 ShortFormSubmitted → pending shortform-coreg verwerken
  // ============================================================
  document.addEventListener("shortFormSubmitted", async () => {
    console.log("🎯 shortFormSubmitted → pending shortform-coreg verwerken...");
    ...
    try {
      ...
      if (!pending.length) {
        const str = sessionStorage.getItem("pendingShortCoreg") || "[]";
        try { pending = JSON.parse(str); } catch { pending = []; }
      }
      ...
      await Promise.allSettled(
        pending.map(async camp => {
          ...
          console.log("➡️ pending coreg payload:", payload);
      ...
      window.pendingShortCoreg = [];
      console.log("✔ Pending shortform-coreg leads verzonden!");

    } catch (e) {
      console.error("❌ pendingShortCoreg fout:", e);
    }
  });
